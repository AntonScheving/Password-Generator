...
// The '= 0;' initializes the variables to zero.
...
function getPasswordOptions() {
  ...
  // the values derived from the prompts are stored inside the characterLengthPrompt variable.
  characterLengthPrompt = parseInt(
    prompt(
      "How many characters do you want in your password? Please choose a number between 10 and 64"
    )
  );
  ...
  while (characterLengthPrompt !== null) {
    // if the user inputs a number below 10 or above 64 or any other character that is not a number the below message will be displayed.
    if (
      isNaN(characterLengthPrompt) ||
      characterLengthPrompt < 10 ||
      characterLengthPrompt > 64
    ) {
      characterLengthPrompt = parseInt(
        prompt(
          "Ooops, it looks like you entered an invalid number or letter. Please enter a number between 10 and 64 to choose the length of your password."
        )
      );
  ...
  if (confirm("Do you want to include special characters in your password?")) {
    // the 'promptChoices' variable is initialized to nothing when it is declared. The = adds the true or false value from the user input in the confirm window as the user clicks either cancel(false) or ok(true) stored in the promptChoices. If the promptChoice variable becomes true the 'concat' combines the character type with the 'promptChoices' variable.
    if (confirm && !promptChoices.includes(specialCharacters)) {
      promptChoices = promptChoices.concat(specialCharacters);
    }
  }
  // else {
  //   // Removes numericCharacters from promptChoices if the user clicks "Cancel"
  //   promptChoices = promptChoices.filter(char => !specialCharacters.includes(char));
  //   }

  if (confirm("Do you want to include numbers in your password?")) {
    if (confirm && !promptChoices.includes(numericCharacters)) {
      promptChoices = promptChoices.concat(numericCharacters);
    }
  }
  // else {
  //   // Removes numericCharacters from promptChoices if the user clicks "Cancel"
  //   promptChoices = promptChoices.filter(char => !numericCharacters.includes(char));
  // }
  ...
  // else {
  //   // Removes numericCharacters from promptChoices if the user clicks "Cancel"
  //   promptChoices = promptChoices.filter(char => !lowerCasedCharacters.includes(char));
  //   }

  if (confirm("Do you want to include uppercase letters in your password?")) {
    if (confirm && !promptChoices.includes(upperCasedCharacters)) {
      promptChoices = promptChoices.concat(upperCasedCharacters);
    }
  }
  // else {
  //   // Removes numericCharacters from promptChoices if the user clicks "Cancel"
  //   promptChoices = promptChoices.filter(char => !upperCasedCharacters.includes(char));
  //   }
  ...
  if (promptChoices.length === 0) {
    alert(
      "Ooops, you must choose at least one character type. Please try again."
    );
  }
...
// After the 'promptChoices' variable has been populated with the character types that the user wants to include in the password in the above function the 'math.random()' element randomizes which characters are used in the password. 'Math.floor' always rounds down and returns the largest integer less than or equal to a given number. '* promptChoices.length' ensures that the randomized characters are taken from the chosen pool of character types chosen by the user in the confirm windows.
function getRandom() {
...
function generatePassword() {
  ...
  for (let i = 0; i < characterLengthPrompt; i++) {
    // The += operator adds the random characters in 'getRandom' to 'displayPassword'.
